...
import { DmrOrderFormData, DmrOrderUpdateDto } from "@/types/orders/DmrOrderTypes"
...
export default function DmrOrderModal ({
    ...
    const { register, watch, control, handleSubmit, formState: { errors}, setValue, reset} = useForm<DmrOrderFormData>({
        defaultValues: {
            label: '',
            assignedUserIds: [],
            orderDate: '',
    ...
    const handleUserClick = (clickedId: number) => {
        ...
        if (!assignedUserIds.includes(clickedId)) setValue('assignedUserIds', [...assignedUserIds, clickedId]) // add to assignments

        else setValue('assignedUserIds', assignedUserIds.filter(assignedId => assignedId != clickedId)) // remove from assignments
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {assignableUsers.map((u, i) => (
                                                    <button
                                                        ...
                                                        className={`px-3 py-1.5 rounded-full font-medium text-sm transition-all ${assignedUserIds.includes(u.id) ? 'bg-blue-100 ring-2 ring-offset-2 ring-blue-500' : 'text-gray-600 bg-gray-100 hover:bg-gray-200'}`}
                                                    >
