...
import {
  createDmrOrderForClientOrder,
  getDmrOrdersByClientOrderId,
} from "@/services/orders/DmrOrderService";
import { toast } from "sonner";
import { useClientOrderRowContext } from "@/context/ClientOrderRowContext";
import {
  DmrOrdersSubTableContext,
  useDmrOrdersSubTableContext,
} from "@/context/DmrOrdersSubTableContext";
...
export default function ClientRowCollapsible() {
  ...
  const { setSelectedDmrRows, selectedClientRows } =
  ...
  const handleUnlink = async (dmrOrderId: number, clientOrderId: number) => {
    ...
    setSelectedDmrRows((prev) =>
      prev.includes(dmrOrderId) ? prev.filter((x) => x !== dmrOrderId) : prev,
    );
  ...
  const showAddDmrOrderButton = hasPermission("dmrOrders", "add");
  ...
  return (
    ...
    >
      ...
      <div className="group grid grid-cols-[40px_1fr] border-b-3 border-gray-200 bg-gray-50/50 dark:border-gray-700 dark:bg-gray-600">
        <div className="relative w-10 flex-shrink-0 border-r border-gray-100 dark:border-gray-700">
          <div
        ...
        <div className="p-4">
          <div className="rounded border border-gray-200 dark:border-gray-700">
            {/* Subheader */}
            <div className="flex items-center justify-between gap-4 border-b border-gray-200 bg-gray-100 px-4 py-2 dark:border-gray-700 dark:bg-gray-700">
              <div className="flex items-center gap-4">
                <ToggleView viewMode={viewMode} onToggle={toggleSubtableView} />
            ...
            {/* Dmr Orders Table */}
            {viewMode == "rows" && <DmrOrdersSubTable />}

            {viewMode == "cards" && (
              <div className="bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
