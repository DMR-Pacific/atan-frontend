...
export function WarnDeleteModal({
  ...
  const {masterClientOrders,masterDmrOrders,masterManufacturerOrders} = useOrdersContext()
  ...
  const formatCurrency = (value: number) => {
    return new Intl.NumberFormat('en-US', {
      ...
      currency: currency,
                 ...
                 const getOrderDetails=(id: OrderId) => {
                    if (tableName == "client_orders")
                    return masterClientOrders[id]
                    else if (tableName == "dmr_orders") 
                    return masterDmrOrders[id]
                    else if (tableName == "manufacturer_orders") 
                    return masterManufacturerOrders[id]
            ...
            <div className="max-h-60 overflow-y-auto rounded-md border border-gray-200 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 shadow-inner">
              <ul role="list" className="divide-y divide-gray-200">
                {orderIds.map((id) => {
                  ...
                  if (tableName == "manufacturer_orders") {
                    orderNumber = (details as ManufacturerOrderMaster).orderNumber
                  }
                    
                  else {
                    orderNumber = (details as ClientOrderMaster | DmrOrderMaster).label
                    value = formatCurrency((details as ClientOrderMaster | DmrOrderMaster).value)
                  }
