...
import { bulkUpdateClientOrderField, updateClientOrderRefColumnsByIdList } from '@/services/orders/ClientOrderService'
...
export function SelectedRowToolbar({
    ...
    const { setDeleteOrderModalOptions, addClientDtosToMaster } = useOrdersContext() 
    const onClose = () => {
    ...
    const handleMoveClientOrdersToGroupId = async (groupId: number) => {
        ...
        try {
            ...
            const bulkFieldUpdateDto: BulkFieldUpdateDto = {
                orderIds: selectedClientRows,
                valueId: groupId
            }
            const response = await bulkUpdateClientOrderField(groupBy, bulkFieldUpdateDto)
            // 4/6/2026, adding to master doesnt update UI because master doesnt update groupings
            // for client orders
