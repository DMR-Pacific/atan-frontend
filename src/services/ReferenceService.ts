...
export const getReference = (refType: ApiRefType) => {
    return atan_api_authorized.get(`${referencesBase}/${refType}`)
}


export const getAssignableUsers = () => {
    return atan_api_authorized.get(`${referencesBase}/assignable-users`)
}

export const createReference = (refType: ApiRefType) => {
    return atan_api_authorized.post(`${referencesBase}/${refType}`)
}

export const updateReference = (refType: ApiRefType, refId: number, refUpdateDto: RefUpdateDto) => {
    return atan_api_authorized.put(`${referencesBase}/${refType}/${refId}`, refUpdateDto)
}

export const deleteReference = (refType: ApiRefType, refId: number) => {
    return atan_api_authorized.delete(`${referencesBase}/${refType}/${refId}`)
}

export const swapReferenceOrder = (refType: ApiRefType, refId1: number, refId2: number) => {
    return atan_api_authorized.put(`${referencesBase}/${refType}/swap-order/${refId1}/${refId2}`)
}